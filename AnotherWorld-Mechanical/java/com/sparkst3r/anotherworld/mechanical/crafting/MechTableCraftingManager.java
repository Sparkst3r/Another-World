...
/**
 * MechTableCraftingManager
 *
 * Manager for the Mechanics Table
 *
 * @author Sparkst3r
 * @since 28 Apr 2014
 */
...
public void addShapelessRecipe(ItemStack par1ItemStack, Object ... par2ArrayOfObj) {
	...
	*/
	...
	public static ItemStack getOutput(InventoryCrafting craftMatrix, World world) {
		...
		for (recipeID = 0; recipeID < craftMatrix.getSizeInventory(); recipeID++) {
			ItemStack stackInSlot = craftMatrix.getStackInSlot(recipeID);
			if (stackInSlot != null) {
				if (var3 == 0) {
					stack1 = stackInSlot;
				}
		 ...
		 */
		if (var3 == 2 && stack1.getItem().getUnlocalizedName().equals(stack2.getItem().getUnlocalizedName()) && stack1.stackSize == 1 && stack2.stackSize == 1 && stack1.getItem().isRepairable()) {
			...
			return new ItemStack(stack1.getItem(), 1, meta);
		}
		 ...
		 */
			...
			for (recipeID = 0; recipeID < recipes.size(); ++recipeID) {
				IRecipe recipe = (IRecipe)recipes.get(recipeID);
	...
	public class MechanicsShapedRecipe implements IRecipe {
		...
		public MechanicsShapedRecipe(int width, int height, ItemStack[] inputStacks, ItemStack outputStack) {
			recipeOutputItem = outputStack.getItem();
			matrixWidth = width;
			matrixHeight = height;
		...
		private boolean checkMatch(InventoryCrafting craftMatrix, int slotX, int slotY, boolean flag) {
			for (int slotAtX = 0; slotAtX < 4; slotAtX++) {
				for (int slotAtY = 0; slotAtY < 4; slotAtY++) {
					...
					if (!itemstack.getItem().getUnlocalizedName().equals(itemstack1.getItem().getUnlocalizedName())) {
						return false;
					}
