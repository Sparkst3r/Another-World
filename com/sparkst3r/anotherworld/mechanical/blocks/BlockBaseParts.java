...
public class BlockBaseParts extends Block implements IToolDismantleable {
	 ...
	 * @param id
	 */
	public BlockBaseParts() {
		super(GlobalValues.IMMOVABLE);
		GameRegistry.registerBlock(this, ItemBlockBaseParts.class, ItemBlockBaseParts.identifier);
	...
	public void registerBlockIcons(IIconRegister ir) {
		iconBuffer = new IIcon[ItemBlockBaseParts.types.length];
		for (int icon = 0; icon < ItemBlockBaseParts.types.length; icon++) {
			iconBuffer[icon] = ir.registerIcon(GlobalValues.TEXTURE + ItemBlockBaseParts.types[icon]);
		}
	}
